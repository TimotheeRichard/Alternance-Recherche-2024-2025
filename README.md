...
## 4. Rejets liquides des centrales nucléaires

Par exemple, dans le cas de la **Moselle**, voici le code qui donne la contribution des **rejets liquides des centrales** dans le modèle :
...
ReadMoselle.df_Parameter.lmig
ReadMoselle.df_Depot
ReadMoselle.update_depot("ReleaseOnly")
ReadMoselle.df_Release.head()
ReadMoselle.df_Carot
ReadMoselle.df_Carot["Cesium137_Bq.kg\n(depose)"]
Mycompute = Concentration(ReadMoselle.df_Parameter, ReadMoselle.df_Depot,ReadMoselle.df_Release, ReadMoselle.df_Carot,Nj)
...
Mycompute.RCT[["pdt","CSED"]].to_csv("DailyModel_ReleaseOnly_Moselle.csv")
Result.to_csv("AveragedModel_ReleaseOnly_Moselle.csv",sep=";",index=False)
```
